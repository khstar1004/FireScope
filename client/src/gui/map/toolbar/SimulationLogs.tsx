...
const simulationLogTypeLabels: Record<SimulationLogType, string> = {
  [SimulationLogType.AIRCRAFT_CRASHED]: "항공기 추락",
  [SimulationLogType.OTHER]: "기타",
  [SimulationLogType.RETURN_TO_BASE]: "기지 복귀",
  [SimulationLogType.STRIKE_MISSION_ABORTED]: "타격 임무 중단",
  [SimulationLogType.STRIKE_MISSION_SUCCESS]: "타격 임무 성공",
  [SimulationLogType.WEAPON_CRASHED]: "무장 추락",
  [SimulationLogType.WEAPON_EXPENDED]: "무장 소진",
  [SimulationLogType.WEAPON_HIT]: "명중",
  [SimulationLogType.WEAPON_LAUNCHED]: "발사",
  [SimulationLogType.WEAPON_MISSED]: "빗나감",
  [SimulationLogType.PATROL_MISSION_SUCCESS]: "초계 임무 유지",
};
...
export default function SimulationLogs(props: SimulationLogsProps) {
          ...
          <FormControl fullWidth sx={{ mb: 2 }}>
            <SelectField
              ...
              selectItems={Object.values(SimulationLogType).map((type) => {
                return {
                  name: simulationLogTypeLabels[type],
                  value: type,
                };
              })}
  ...
  const createMessageLogComponent = (log: SimulationLog) => {
    ...
    const sideName = side?.name ?? "알 수 없음";
            ...
            action={
              <>
                <Stack direction="row" spacing={1}>
                  <Tooltip title="기록 필터" placement="top" arrow>
                    <IconButton
                      id="filter-button"
                      aria-controls={
