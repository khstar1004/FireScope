  ...
  const defaultCardActions = (
    <Stack spacing={0.5} direction="column" onMouseLeave={handleClose}>
      ...
      <ListItemButton onClick={_handleArmyAutoAttack}>
        <RocketLaunchIcon sx={{ mr: 0.5 }} /> 자동 공격
      </ListItemButton>
      <ListItemButton onClick={toggleWeapons}>
        <Radar sx={{ mr: 0.5 }} /> 수동 공격
      </ListItemButton>
            ...
            <>
              {cardContentContext === "default" && (
                <Stack direction={"row"} spacing={0}>
                  <Tooltip title={`${props.army.name} 편집`}>
                    <IconButton onClick={toggleEdit}>
                      <EditIcon sx={{ color: "white" }} />
                    </IconButton>
                  </Tooltip>
                  <Tooltip title={`${props.army.name} 삭제`}>
          ...
          subheader={
            ...
            >
              <Typography variant="caption">
                유형: {getDisplayName(props.army.className)}
              </Typography>
              <Typography variant="caption">
                세력:{" "}
