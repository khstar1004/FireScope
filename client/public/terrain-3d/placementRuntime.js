...
function expandBounds(bounds, ratio = 0.04, minimumPadding = 0.0005) {
  const widthPadding = Math.max((bounds.east - bounds.west) * ratio, minimumPadding);
...
function hasGroundModelBudget(renderContext, facilityModelBudget = GROUND_MODEL_BUDGET) {
...
function resolveUnitModel(unit, renderContext) {
  ...
  const resolvedByProfileId = resolveUnitModelFromProfileId(unit, renderContext);
...
function createPlacementLabel(Cesium, unit, emphasized, labelDistance) {
  return {
    text: truncateText(unit?.name ?? unit?.className ?? "이름 없음", emphasized ? 34 : 24),
    scale: emphasized ? 0.74 : 0.6,
    showBackground: true,
    backgroundColor: colorForSide(Cesium, unit?.sideColor, emphasized ? 0.24 : 0.12),
...
function resolveWeaponEffectProfile(weapon) {
  const signature =
    `${weapon?.modelId ?? ""} ${weapon?.className ?? ""} ${weapon?.name ?? ""}`.toLowerCase();
  const artilleryLike =
  ...
  return {
    ...
    trailWidth: artilleryLike ? 8.8 : bulletLike ? 5.8 : interceptorLike ? 7.2 : 7.8,
    projectedWidth: artilleryLike ? 3.6 : bulletLike ? 2.6 : 3.2,
    glowScale: artilleryLike ? 1.46 : bombLike ? 1.34 : bulletLike ? 1.16 : 1.24,
  };
}

function createWeaponPointGraphics(Cesium, weapon) {
  const highAltitudeWeapon = Math.max(0, Number(weapon?.altitudeMeters) || 0) > 400;
  const effectProfile = resolveWeaponEffectProfile(weapon);

  return {
    pixelSize: effectProfile.bulletLike ? 12 : highAltitudeWeapon ? 17 : 14,
    color: colorForSide(Cesium, weapon?.sideColor, 0.96),
...
function createWeaponBillboardGraphics(Cesium, weapon) {
  ...
  return {
    ...
    width: effectProfile.artilleryLike ? 58 : effectProfile.bulletLike ? 42 : 50,
    height: effectProfile.artilleryLike ? 58 : effectProfile.bulletLike ? 42 : 50,
    disableDepthTestDistance: Number.POSITIVE_INFINITY,
  };
}

function createWeaponLabelGraphics(Cesium, weapon) {
  const effectProfile = resolveWeaponEffectProfile(weapon);
  return {
    text: effectProfile.label,
    ...
    outlineWidth: 3,
    showBackground: true,
    backgroundColor: colorForSide(Cesium, weapon?.sideColor, 0.54),
...
function sampleWeaponArc(startPoint, endPoint, ratio, apexHeightMeters) {
  ...
  return {
    ...
    latitude: startPoint.latitude + (endPoint.latitude - startPoint.latitude) * t,
    altitudeMeters:
...
function buildWeaponArcPointSamples(weapon, endPoint, sampleCount = WEAPON_ARC_SAMPLE_COUNT) {
...
function buildWeaponTrailPositions(Cesium, weapon) {
  ...
  const progress = fullDistance > 1 ? clamp01(currentDistance / fullDistance) : 1;
  const sampleCount = Math.max(4, Math.round(WEAPON_ARC_SAMPLE_COUNT * progress));
...
function createEventTraceGlowGraphics(Cesium, traceWeapon, positions, freshness) {
  const effectProfile = resolveWeaponEffectProfile(traceWeapon);
  return {
    positions,
    width: effectProfile.artilleryLike ? 7.2 : effectProfile.bulletLike ? 4.8 : 6.2,
    material: new Cesium.PolylineGlowMaterialProperty({
      glowPower: effectProfile.artilleryLike ? 0.36 : 0.3,
      taperPower: 0.85,
      color: colorForSide(Cesium, traceWeapon?.sideColor, 0.22 + freshness * 0.5),
    }),
    clampToGround: false,
    disableDepthTestDistance: Number.POSITIVE_INFINITY,
  };
...
function createEventTraceDashGraphics(Cesium, traceWeapon, positions, freshness) {
  const effectProfile = resolveWeaponEffectProfile(traceWeapon);
  return {
    positions,
    width: effectProfile.artilleryLike ? 3.4 : effectProfile.bulletLike ? 2.4 : 2.9,
    material: new Cesium.PolylineDashMaterialProperty({
      color: colorForSide(Cesium, traceWeapon?.sideColor, 0.36 + freshness * 0.42),
...
function buildEventDescription(event) {
  return [
    ...
    event?.actorName
      ? `행위자 ${escapeHtml(event.actorName)}`
      : null,
    event?.targetName
      ? `대상 ${escapeHtml(event.targetName)}`
      : null,
  ]
    .filter(Boolean)
    .join("<br/>");
...
export function createTerrainPlacementRuntime({
  ...
  function makeEffectTimelineColorProperty(effect, cssColor, stops, fallback = 0) {
    return new Cesium.CallbackProperty(() => {
  ...
  function spawnLaunchEffect(effectKey, weapon) {
    if (!runtimeVisualOptions.showEventEffects || activeTransientEffects.has(effectKey)) {
      return;
    }
  ...
  function spawnImpactEffect(effectKey, point, sideColor = "yellow", options = {}) {
    if (!runtimeVisualOptions.showEventEffects || activeTransientEffects.has(effectKey)) {
      return;
    }
    ...
    const impactColor = SIDE_COLOR_MAP[`${sideColor ?? ""}`.toLowerCase()] ?? "#ff9f1c";
    const baseAltitude = Math.max(0, Number(point.altitudeMeters) || 0);
    const radiusMeters = options.radiusMeters ?? 118;
    ...
    activeTransientEffects.set(effectKey, effect);
    startTransientEffectLoop();
    triggerImpactShake(options.shakeIntensity ?? 10, options.shakeDurationMs ?? 260);
  }
  ...
  function renderRuntimeEvents(events = []) {
    ...
    runtimeEventList.innerHTML = events
      .map((event) => {
        const sideTone =
          SIDE_COLOR_MAP[`${event?.sideColor ?? ""}`.toLowerCase()] ?? "#7fe7ff";
        return `
  ...
  function renderRuntimeSummary(snapshot, visibleUnits, visibleWeapons, visibleEvents) {
    if (runtimeModeBadge) {
  ...
  function upsertUnitState(unit, renderContext) {
    ...
    if (!existingState) {
      const entityConfig = {
        ...
        position,
        orientation: resolveHeadingOrientation(Cesium, displayPoint, unit?.headingDeg),
  ...
  function syncFocusedVisuals(previousFocusedUnitId, nextFocusedUnitId) {
    const changedUnitIds = new Set(
      [previousFocusedUnitId, nextFocusedUnitId, lastSnapshot?.selectedUnitId].filter(
  ...
  function upsertWeaponState(weapon) {
      ...
      const trailEntity =
        runtimeVisualOptions.showWeaponTrails && trailPositions.length > 0
          ? weaponDataSource.entities.add({
              ...
              polyline: createWeaponTravelTrailGraphics(Cesium, weapon, trailPositions),
            })
          : null;
    ...
    if (runtimeVisualOptions.showWeaponTrails && trajectoryPositions.length > 0) {
  ...
  function resolveGroundAbsoluteAltitude(displayPoint) {
    ...
    if (!Number.isFinite(longitude) || !Number.isFinite(latitude) || !viewer?.scene) {
  ...
  function focusUnitById(unitId, options = {}) {
    ...
    renderRuntimeSummary(
      ...
      Array.isArray(lastSnapshot?.recentEvents) ? lastSnapshot.recentEvents.slice(0, 6) : []
    );
    requestRender();
  }
  ...
  function applySnapshot(snapshot) {
    ...
    const nextFocusedUnitId = visibleUnits.some((unit) => unit.id === focusedUnitId)
      ? focusedUnitId
    ...
    renderAssetPanel();
    renderRuntimeSummary(snapshot, visibleUnits, visibleWeapons, fallbackEvents);
    if (focusedUnitId && unitStateById.has(focusedUnitId)) {
      viewer.selectedEntity = unitStateById.get(focusedUnitId)?.entity ?? undefined;
    }
    requestRender();
  }
