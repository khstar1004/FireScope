...
import {
  ...
  shouldPreferVWorldRuntime,
} from "./world/cesiumWorld";
import { isGlobeSurfaceReady } from "./world/globeReadiness";
...
const requestedLon = Number(
  urlParams.get("lon") ?? runtimeConfig.initialPosition?.lon
);
const requestedLat = Number(
  urlParams.get("lat") ?? runtimeConfig.initialPosition?.lat
);
const requestedAlt = Number(
  urlParams.get("alt") ?? runtimeConfig.initialPosition?.alt
);
...
const hasVWorldRuntimeConfig = Boolean(
  (runtimeConfig.vworldApiKey ?? "").trim()
);
const hasMapTilerRuntimeConfig = Boolean(
  (runtimeConfig.mapTilerApiKey ?? "").trim()
);
...
const resolveFlightSimStartPosition = (lon, lat) =>
  Number.isFinite(lon) && Number.isFinite(lat) && isInsideKorea(lon, lat)
    ? { lon, lat }
    : { ...DEFAULT_KOREA_START };
const requestedStartPosition = resolveFlightSimStartPosition(
  requestedLon,
  requestedLat
);
const shouldStartWithVWorldRuntime = shouldPreferVWorldRuntime({
...
let activeCraft = normalizeCraftProfile(
  getCraftProfile(
    resolveRequestedCraftId(requestedCraft, gameSettings.selectedJetCraftId)
  )
);
...
function normalizeCraftProfile(craftProfile) {
  ...
  return {
    ...
    iconPath:
      typeof sourceCraft.iconPath === "string" &&
      sourceCraft.iconPath.length > 0
...
function updateSettingsUI() {
  ...
  document.getElementById("graphicsQuality").value =
  ...
  document.getElementById("showHorizonLines").checked =
    gameSettings.showHorizonLines;
  document.getElementById("soundEnabled").checked = gameSettings.soundEnabled;
  document.getElementById("minimapRange").value =
    gameSettings.minimapRange.toString();
...
function applySettings() {
  ...
  if (soundManager && soundManager.listener) {
    soundManager.listener.setMasterVolume(
      gameSettings.soundEnabled ? 1.0 : 0.0
    );
  }
  ...
  if (viewer) {
    if (gameSettings.graphicsQuality === "low") {
      ...
      viewer.scene.globe.maximumScreenSpaceError = 1.45;
    } else {
...
let state = {
  lon: requestedStartPosition.lon,
  lat: requestedStartPosition.lat,
  alt: Number.isFinite(requestedAlt)
    ? requestedAlt
    : activeCraft.initialAltitude,
  heading: 0,
  pitch: 0,
  roll: 0,
...
function applyMissileCameraView(
  missile,
  label = CAMERA_MODE_LABELS[CAMERA_MODES.MISSILE]
) {
      ...
      return {
        ...
        name:
          typeof platform?.name === "string" && platform.name.trim().length > 0
            ...
            : "집중포격 발사대",
        className:
          typeof platform?.className === "string" &&
          platform.className.trim().length > 0
      ...
      return {
        ...
        launcherId:
          typeof track?.launcherId === "string" && track.launcherId.length > 0
            ...
            : "focus-fire-launcher",
        launcherName:
          typeof track?.launcherName === "string" &&
          track.launcherName.trim().length > 0
...
function updateFocusFireMainControl() {
  if (
    !focusFireMainControl ||
    !focusFireMainStatus ||
    !focusFireMainObjective ||
    !focusFireMainMetrics ||
    !focusFireMainStartBtn
  ) {
    return;
  }
  ...
  const metricsText = `탄체 ${focusFireRuntimeState.weaponsInFlight} · 점령 ${Math.round(focusFireRuntimeState.captureProgress)}% · 포대 ${focusFireRuntimeState.artilleryCount} / 기갑 ${focusFireRuntimeState.armorCount} / 항공 ${focusFireRuntimeState.aircraftCount}`;
...
function triggerFocusFireMainBarrage(
  bursts = null,
  focusState = focusFireRuntimeState
) {
  if (!focusState) {
    return;
  }
  ...
  if (
    ...
    if (focusFireSystem && (commandState ?? focusFireRuntimeState)) {
      triggerFocusFireMainBarrage(
        bursts,
        commandState ?? focusFireRuntimeState
      );
    } else {
...
function updateCraftHelp() {
  ...
  if (helpTitle) {
    helpTitle.textContent =
      activeCraft.mode === "drone" ? "드론 안내" : "비행 안내";
  }
...
function initializeCraftModel(mesh, animations = []) {
  ...
  const {
    ...
    y: baseRotY,
    z: baseRotZ,
  } = activeCraft.visual.baseRotation;
...
function loadCraftModel(loader, modelCandidates, onLoad, onError, index = 0) {
  ...
  if (index >= validCandidates.length) {
    onError(
      new Error(`Unable to load any model from: ${validCandidates.join(", ")}`)
    );
    return;
  }
...
const focusFireMainObjective = document.getElementById(
  "focus-fire-main-objective"
);
...
let battleSpectatorHud = null;

const BATTLE_SPECTATOR_CAMERA_LABELS = {
  tactical: "전술 시야",
  side: "측면 시야",
  chase: "추적 시야",
  orbit: "오비트 시야",
};
const BATTLE_SPECTATOR_LOD_LABELS = {
  cinematic: "영상형",
  balanced: "균형",
  performance: "성능",
};
...
function ensureFlightSimRuntimeState() {
  const runtime = window.__FLIGHT_SIM_RUNTIME__ ?? {};
  runtime.vworld = runtime.vworld ?? {};
  runtime.startup = runtime.startup ?? {};
  window.__FLIGHT_SIM_RUNTIME__ = runtime;
  return runtime;
}

function resolveFlightSimStartupStatus() {
  const battleSpectatorOverlayMode = isBattleSpectatorOverlayMode();
  const commonState = {
    battleSpectatorOverlayMode,
    battleSpectatorSessionStarted,
    currentState,
    audioReady: loadingStatus.audio,
    modelReady: loadingStatus.model,
    cesiumReady: loadingStatus.cesium,
    globeReady: loadingStatus.globe,
  };

  if (battleSpectatorOverlayMode) {
    const readyForSimulation =
      loadingStatus.cesium && loadingStatus.globe && !loadingStatus.failed;
    const failed =
      loadingStatus.failed && (!loadingStatus.cesium || !loadingStatus.globe);
    const loadingMessage = failed
      ? "전장 관전자 3D를 불러오지 못했습니다. 새로고침해 주세요."
      : readyForSimulation
        ? "전장 관전자 시점을 여는 중..."
        : !loadingStatus.cesium
          ? "위성 지도를 불러오는 중..."
          : "지형 표면을 불러오는 중...";

    return {
      ...commonState,
      phase: failed ? "failed" : readyForSimulation ? "ready" : "loading",
      failed,
      loadingMessage,
      readyForSimulation,
    };
  ...
  const loadingMessage = loadingStatus.failed
    ? "불러오기에 실패했습니다. 새로고침해 주세요."
    : !loadingStatus.audio
      ? "오디오를 불러오는 중..."
      : !loadingStatus.model
        ? activeCraft.loadingLabel
        : !loadingStatus.cesium
          ? "위성 지도를 불러오는 중..."
          : !loadingStatus.globe
            ? "지형 표면을 불러오는 중..."
...
function publishFlightSimStartupStatus() {
  const runtime = ensureFlightSimRuntimeState();
  runtime.startup = {
    ...(runtime.startup ?? {}),
    ...resolveFlightSimStartupStatus(),
  };
  window.__FLIGHT_SIM_RUNTIME__ = runtime;
  updateBattleSpectatorHud();
}

function ensureBattleSpectatorHud() {
  if (battleSpectatorHud || !document.body) {
    return battleSpectatorHud;
  }

  const root = document.createElement("div");
  root.id = "battleSpectatorHud";
  root.classList.add("hidden");
  root.innerHTML = `
    <div class="battle-spectator-panel">
      <div class="battle-spectator-kicker-row">
        <span class="battle-spectator-kicker">3D BATTLE SPECTATOR</span>
        <span id="battleSpectatorStageChip" class="battle-spectator-chip" data-tone="loading">INITIALIZING</span>
      </div>
      <div id="battleSpectatorHudTitle" class="battle-spectator-title">전장 관전자</div>
      <div id="battleSpectatorHudSummary" class="battle-spectator-summary">전장 데이터를 정렬하는 중입니다.</div>
      <div class="battle-spectator-metrics">
        <span id="battleSpectatorHudUnits" class="battle-spectator-metric">유닛 0</span>
        <span id="battleSpectatorHudWeapons" class="battle-spectator-metric">탄체 0</span>
        <span id="battleSpectatorHudEvents" class="battle-spectator-metric">이벤트 0</span>
      </div>
    </div>
    <div class="battle-spectator-ticker">
      <div class="battle-spectator-ticker-row">
        <span class="battle-spectator-ticker-label">FOCUS</span>
        <span id="battleSpectatorHudFocus" class="battle-spectator-ticker-value">광역 시야</span>
      </div>
      <div class="battle-spectator-ticker-row">
        <span class="battle-spectator-ticker-label">VIEW</span>
        <span id="battleSpectatorHudView" class="battle-spectator-ticker-value">전술 시야 · 균형</span>
      </div>
      <div class="battle-spectator-ticker-row">
        <span class="battle-spectator-ticker-label">LATEST</span>
        <span id="battleSpectatorHudLatest" class="battle-spectator-ticker-value">새 이벤트 대기 중</span>
      </div>
    </div>
  `;
  ...
  battleSpectatorHud = {
    root,
    stageChip: root.querySelector("#battleSpectatorStageChip"),
    title: root.querySelector("#battleSpectatorHudTitle"),
    summary: root.querySelector("#battleSpectatorHudSummary"),
    units: root.querySelector("#battleSpectatorHudUnits"),
    weapons: root.querySelector("#battleSpectatorHudWeapons"),
    events: root.querySelector("#battleSpectatorHudEvents"),
    focus: root.querySelector("#battleSpectatorHudFocus"),
    view: root.querySelector("#battleSpectatorHudView"),
    latest: root.querySelector("#battleSpectatorHudLatest"),
  };
  return battleSpectatorHud;
}

function resolveBattleSpectatorFocusLabel(snapshot) {
  const followTargetId = snapshot?.view?.followTargetId;
  if (typeof followTargetId !== "string" || followTargetId.length === 0) {
    return "광역 시야";
  }

  const normalizedTargetId = followTargetId.startsWith("weapon:")
    ? followTargetId.slice("weapon:".length)
    : followTargetId.startsWith("unit:")
      ? followTargetId.slice("unit:".length)
      : followTargetId;
  const trackedWeapon = (snapshot?.weapons ?? []).find(
    (weapon) => weapon.id === normalizedTargetId
  );
  if (trackedWeapon) {
    return trackedWeapon.name;
  }

  return (
    (snapshot?.units ?? []).find((unit) => unit.id === normalizedTargetId)?.name ??
    "자동 포착"
  );
}

function resolveBattleSpectatorStage(snapshot) {
  if (!snapshot) {
    return {
      label: "INITIALIZING",
      tone: "loading",
      summary: "전장 피드와 지형을 정렬하는 중입니다.",
    };
  }

  const weaponCount = snapshot.weapons?.length ?? 0;
  const eventCount = snapshot.recentEvents?.length ?? 0;
  ...
  if (weaponCount > 0) {
    return {
      label: "LIVE FIRE",
      tone: "hot",
      summary: `유닛 ${unitCount} · 탄체 ${weaponCount} · 최근 이벤트 ${eventCount}`,
    };
  }
  ...
  if (eventCount > 0) {
    return {
      label: "CONTACT",
      tone: "contact",
      summary: `유닛 ${unitCount} · 최근 교전 ${eventCount}`,
    };
  }
  ...
  return {
    label: "SURVEILLANCE",
    tone: "calm",
    summary: `유닛 ${unitCount} · 교전 대기`,
  };
}

function updateBattleSpectatorHud() {
  const hud = ensureBattleSpectatorHud();
  if (!hud) {
    return;
  }
  ...
  const snapshot = battleSpectatorRuntimeState;
  const latestEventMessage =
    snapshot?.recentEvents?.[snapshot.recentEvents.length - 1]?.message ??
    "새 이벤트 대기 중";
  const stage = resolveBattleSpectatorStage(snapshot);
  const startupStatus = window.__FLIGHT_SIM_RUNTIME__?.startup;
  const loadingSummary =
    startupStatus?.phase === "loading" ? startupStatus.loadingMessage : null;
  const cameraLabel =
    BATTLE_SPECTATOR_CAMERA_LABELS[snapshot?.view?.cameraProfile] ??
    "전술 시야";
  const lodLabel =
    BATTLE_SPECTATOR_LOD_LABELS[snapshot?.view?.lodLevel] ?? "균형";

  hud.stageChip.textContent = stage.label;
  hud.stageChip.dataset.tone = stage.tone;
  hud.title.textContent = snapshot?.scenarioName ?? "전장 관전자";
  hud.summary.textContent = loadingSummary ?? stage.summary;
  hud.units.textContent = `유닛 ${snapshot?.units?.length ?? 0}`;
  hud.weapons.textContent = `탄체 ${snapshot?.weapons?.length ?? 0}`;
  hud.events.textContent = `이벤트 ${snapshot?.recentEvents?.length ?? 0}`;
  hud.focus.textContent = resolveBattleSpectatorFocusLabel(snapshot);
  hud.view.textContent = `${cameraLabel} · ${lodLabel}`;
  hud.latest.textContent = latestEventMessage;
}
...
function distanceMetersBetweenBattlePoints(sourcePoint, targetPoint) {
...
function bearingDegreesBetweenBattlePoints(sourcePoint, targetPoint) {
  const sourceLongitude = Number(sourcePoint?.longitude) || 0;
  const sourceLatitude = Number(sourcePoint?.latitude) || 0;
  const targetLongitude = Number(targetPoint?.longitude) || 0;
  const targetLatitude = Number(targetPoint?.latitude) || 0;
  const latitudeRadians = Cesium.Math.toRadians(
    (sourceLatitude + targetLatitude) * 0.5
  );
  const eastMeters =
    (targetLongitude - sourceLongitude) *
    111320 *
    Math.max(Math.cos(latitudeRadians), 0.01);
  const northMeters = (targetLatitude - sourceLatitude) * 110540;
  return Cesium.Math.toDegrees(Math.atan2(eastMeters, northMeters));
}

function collectBattleSpectatorOverviewPoints() {
  const points = [];
  const appendPoint = (longitude, latitude, altitudeMeters = 0) => {
    if (
  ...
  (battleSpectatorRuntimeState?.units ?? []).slice(0, 64).forEach((unit) => {
    appendPoint(unit.longitude, unit.latitude, unit.altitudeMeters);
  });
  (battleSpectatorRuntimeState?.weapons ?? []).slice(-32).forEach((weapon) => {
    appendPoint(weapon.longitude, weapon.latitude, weapon.altitudeMeters);
  });
...
function resolveBattleSpectatorOverview() {
  const latestEventWithPoint = Array.isArray(
    battleSpectatorRuntimeState?.recentEvents
  )
    ? [...battleSpectatorRuntimeState.recentEvents]
        .reverse()
  ...
  const latestEventSource =
    latestEventWithPoint &&
    Number.isFinite(Number(latestEventWithPoint?.sourceLongitude)) &&
    Number.isFinite(Number(latestEventWithPoint?.sourceLatitude))
      ? {
          longitude: Number(latestEventWithPoint.sourceLongitude),
          latitude: Number(latestEventWithPoint.sourceLatitude),
          altitudeMeters: Math.max(
            0,
            Number(latestEventWithPoint.sourceAltitudeMeters) || 0
          ),
        }
      : null;
  const overviewPoints = collectBattleSpectatorOverviewPoints();
  ...
  const resolvedCenter = {
    longitude:
      !latestEventWithPoint && averagedPoint
        ? averagedPoint.longitude / overviewPoints.length
        : longitude,
    latitude:
      !latestEventWithPoint && averagedPoint
        ? averagedPoint.latitude / overviewPoints.length
        : latitude,
    altitudeMeters: Math.max(
      0,
  ...
  const spanMeters = overviewPoints.reduce((maxDistance, point) => {
    return Math.max(
      maxDistance,
      distanceMetersBetweenBattlePoints(resolvedCenter, point)
    );
  }, 0);
  const altitudeMeters = Math.min(
    ...
    Math.max(
      3200,
      resolvedCenter.altitudeMeters + 1400,
      spanMeters * 0.72,
      state.alt + activeCraft.spawnAltitudeOffset + 900
    )
  );
  const headingDegrees = latestEventSource
    ? bearingDegreesBetweenBattlePoints(latestEventSource, resolvedCenter)
    : 18;
  const pitchDegrees =
    spanMeters >= 22000
      ? -66
...
function startBattleSpectatorSession() {
  ...
  viewer.camera.flyTo({
    ...
    orientation: {
      heading: Cesium.Math.toRadians(overview.headingDegrees ?? 18),
      pitch: Cesium.Math.toRadians(overview.pitchDegrees ?? -55),
      roll: 0,
    },
    duration: 1.35,
...
async function initSounds() {
  ...
  const soundDefinitions = [
    {
      name: "boost",
      path: "assets/sounds/boost.mp3",
      loop: false,
      volume: 0.35,
    },
    {
      name: "throttle",
      path: "assets/sounds/throttle.mp3",
      loop: false,
      volume: 0.4,
    },
    {
      name: "explode",
      path: "assets/sounds/explode.mp3",
      loop: false,
      volume: 0.75,
    },
    {
      name: "explosion-1",
      path: "assets/sounds/explosion-1.mp3",
      loop: false,
      volume: 0.8,
    },
    {
      name: "explosion-2",
      path: "assets/sounds/explosion-2.mp3",
      loop: false,
      volume: 0.8,
    },
    {
      name: "explosion-3",
      path: "assets/sounds/explosion-3.mp3",
      loop: false,
      volume: 0.8,
    },
    {
    ...
    {
      name: "weapon-warning",
      path: "assets/sounds/weapon-warning-1.mp3",
      loop: false,
      volume: 1.0,
    },
    {
      name: "jet-engine",
      path: "assets/sounds/jet-engine.mp3",
      loop: true,
      volume: 0.5,
    },
    {
      name: "spawn",
      path: "assets/sounds/spawn.mp3",
      loop: false,
      volume: 0.5,
    },
    ...
    {
      name: "pitch",
      path: "assets/sounds/pitch.mp3",
      loop: true,
      volume: 0.75,
    },
    {
      name: "button-click",
      path: "assets/sounds/button-click.mp3",
      loop: false,
      volume: 1.0,
    },
    {
    ...
    {
      name: "button-hover",
      path: "assets/sounds/button-hover.mp3",
      loop: false,
      volume: 0.25,
    },
    {
      name: "zoom-in",
      path: "assets/sounds/zoom-in.mp3",
      loop: false,
      volume: 0.5,
    },
    {
      name: "missile-fire",
      path: "assets/sounds/missile-firing-1.mp3",
      loop: false,
      volume: 0.75,
    },
    {
      name: "m61-firing",
      path: "assets/sounds/m61-firing.mp3",
      loop: true,
      volume: 0.75,
    },
    {
      name: "rwr-tws",
      path: "assets/sounds/rwr-tws.mp3",
      loop: true,
      volume: 0.2,
    },
    {
      name: "rwr-lock",
      path: "assets/sounds/rwr-lock.mp3",
      loop: false,
      volume: 0.2,
    },
    { name: "wind", path: "assets/sounds/wind.mp3", loop: true, volume: 0.25 },
    {
      ...
      path: "assets/sounds/terrain-pull-up.mp3",
      loop: false,
      volume: 0.9,
    },
    {
      name: "warning",
      path: "assets/sounds/warning.mp3",
      loop: false,
      volume: 0.6,
    },
    {
      name: "glitch-1",
      path: "assets/sounds/glitch-transition-1.mp3",
      loop: false,
      volume: 0.25,
    },
    {
      name: "glitch-2",
      path: "assets/sounds/glitch-transition-2.mp3",
      loop: false,
      volume: 0.25,
    },
    {
      name: "glitch-3",
      path: "assets/sounds/glitch-transition-3.mp3",
      loop: false,
      volume: 0.25,
    },
    {
      name: "glitch-4",
      path: "assets/sounds/glitch-transition-4.mp3",
      loop: false,
      volume: 0.25,
    },
  ];
  ...
  if (failedSounds.length > 0) {
    console.error(
      "Failed to load one or more flight simulator sounds.",
      failedSounds
    );
  }
  ...
  document.addEventListener(
    ...
    (e) => {
      const target = e.target.closest(
        "button, .menu-btn, .clickable-ui, #search-toggle-btn"
      );
      if (target) {
...
function initThree() {
  ...
  loadCraftModel(
    ...
    (error) => {
      ...
      if (fallbackModel) {
        console.warn(
          "Drone GLB asset not found. Using built-in fallback drone model.",
          error
        );
...
function update(dt) {
  ...
  if (
    nowTime - lastGeocodeTime > GEOCODE_INTERVAL ||
    distFromLast > GEOCODE_MIN_DIST
  ) {
  ...
  if (activeCraft.enableJetAudio && soundManager.isPlaying("jet-engine")) {
    ...
    const speedFactor = Math.max(
      0,
      Math.min(1.0, (state.speed - minSpeed) / (maxSpeed - minSpeed))
    );
  ...
  if (
    activeCraft.enableJetAudio &&
    (Math.abs(input.roll) > 0.5 || Math.abs(input.yaw) > 0.5)
  ) {
  ...
  hud.update(
    state,
    currentState === States.FLYING ? (npcSystem ? npcSystem.npcs : []) : []
  );

  if (planeModel) {
    const accel = (state.speed - prevSpeed) / dt;
    const accelInertia = input.isDragging
      ? 0
      : Math.max(-0.5, Math.min(1.5, accel * 0.001));
    ...
    if (activeCraft.enableBoost && physicsResult.isBoosting) {
      ...
      const p = Math.max(
        0,
        Math.min(1.0, 1.0 - physicsResult.boostTimeRemaining / T)
      );

      const totalRotationRad =
        Math.PI * 2 * physicsResult.boostRotations * boostRollDirection;
      ...
      if (p < 0.2) {
        ...
        boostRoll =
          easedP *
          (Math.PI * 2 * physicsResult.boostRotations) *
          boostRollDirection;
      } else {
    ...
    const idleX =
      Math.sin(time * 0.8) * (activeCraft.mode === "drone" ? 0.015 : 0.035);
    const idleY =
      Math.cos(time * 0.6) * (activeCraft.mode === "drone" ? 0.012 : 0.025);
    const idleRotX =
      Math.sin(time * 0.5) * (activeCraft.mode === "drone" ? 0.01 : 0.015);
    const idleRotY =
      Math.cos(time * 0.4) * (activeCraft.mode === "drone" ? 0.01 : 0.015);
    const idleRotZ =
      Math.sin(time * 0.7) * (activeCraft.mode === "drone" ? 0.012 : 0.025);
    ...
    const lerpFactor =
      activeCraft.enableBoost && physicsResult.isBoosting
        ? 3.0 * dt
        : activeCraft.mode === "drone"
          ? 7.0 * dt
          : 5.0 * dt;
...
function checkGPWS() {
  ...
  if (showWarning) {
    ...
    if (
      !gpwsActive ||
      (now - lastGPWSWarningTime > GPWS_COOLDOWN &&
...
function animate() {
  ...
  if (
    ...
    if (battleSpectatorSystem) {
      battleSpectatorSystem.update(
        currentState === States.FLYING ? dt : dt * 0.3
      );
    }
    updateFocusFireMainControl();
...
function setupModalListeners() {
  ...
  document.getElementById("saveOptionsBtn").onclick = () => {
    ...
    gameSettings.mouseSensitivity = parseFloat(
      document.getElementById("sensitivitySlider").value
    );
    ...
    gameSettings.minimapRange = parseInt(
      document.getElementById("minimapRange").value,
      10
    );
...
async function bootstrap() {
  const viewer = await initCesium({
    lon: state.lon,
    lat: state.lat,
    alt: shouldStartWithVWorldRuntime
  ...
  const updateGlobeSurfaceReadiness = () => {
    if (loadingStatus.globe) {
      return true;
    }

    if (!isGlobeSurfaceReady(viewer, window.__FLIGHT_SIM_RUNTIME__)) {
      return false;
    }
  ...
  if (!updateGlobeSurfaceReadiness()) {
